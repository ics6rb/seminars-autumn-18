...
Большинство текстовых редакторов имеют режим автодополнения и способны генерировать шаблонный код. Попытаемся сгенерировать HTML-разметку, введя `html` в открытом файле `index.html` (в вашем VS Code должен быть установлен пакет `HTML-Snippets`). Мы увидим следующее:
...
Элемент `<meta>` - это контейнер для метаинформации. В частности, можно задать кодировку символов с помощью `<meta charset="utf-8" />`. Конструкция `<meta name="viewport" content="width=device-width, initial-scale=1">` заставляет браузер подгонять размеры страницы под размер экрана устройства, чтобы пользователю смартфона не пришлось увеличивать контент и скроллить его после каждой загрузки страницы.
...
Разделим страницу на две части - заголовок и основную часть. В HTML существует элемент `<div>` - фактически это прямоугольник для ограничения какой-либо области. Элемент `<div>`  в свое время использовался повсеместно, поэтому лет 5 назад было бы нормально написать так:
...
Элемент `<input>`, как можно заметить, может выступать в роли как поля ввода, так и кнопки подтверждения. Однако в HTML есть элемент `<button>`, который является "настоящей" кнопкой. Мы можем использовать его, при этом все будет выглядеть и работать так же (главное не забыть указать `type="submit"`):
...
1. Он разделяет разметку и ее отображение (его часто называют стилем). За разметку отвечает язык HTML, а за стили - CSS.
2. Он позволяет применять один и тот же стиль к нескольким элементам (и, как следствие, менять стиль нескольких элементов сразу).
...
Этот вариант будет работать, но он не лучший. Дело в том, что теперь ни в каком ненумерованном списке не будут отображаться точки, а это может нас не устраивать. Гораздо правильнее воспользовать атрибутами class или id. В рамках нашей задачи, вероятно, этот список будет уникальным, поэтому присвоим ему id:
...
Здесь символ `#` - селектор элемента по id.
...
Все, что заключено в фигурные скобки - необязательные части URL.
...
* только фрагмент path: `<form action="path/fragment">` (`/` в начале стоять не должен!), в таком случае браузер подставит scheme и path с текущей страницы. Иными словами, если форма находится на http://www.example.com/form и `action="path/fragment"`, то мы перейдем на http://www.example.com/path/fragment.
...
У элемента `<form>` еще есть атрибут method - он указывает, каким HTTP-методом (что это такое и зачем они нужны, мы рассмотрим позже) нужно отправлять форму. Но вот загвоздка, этот атрибут не является обязательным и у него есть значение по умолчанию - `GET`... Может быть, нам просто было нечего отправлять?

### В каком виде происходит отправка данных?

У нас есть много полей ввода, в которые пользователь вбивает информацию. Как бы вы организовали работу с этими полями, если бы были разработчиками браузеров? Можно было бы, конечно, просто записать пользовательский ввод в виде строк в массив и отдать программисту, но это не настолько же удобно, насколько просто в реализации. Гораздо удобнее представлять эти данные в виде "ключ - значение", где значение - это то, что ввел пользователь, а ключ - это какое-то произвольное название поля, которое программист даст ему для своего удобства.

Звучит неплохо, но тогда к тегу `<input>` нужно добавить какой-то атрибут с названием этого поля ввода. Такой атрибут есть и он называется, как ни странно, `name`. В этом-то и была наша проблема - браузер видел, что ни одного поле ввода не содержит атрибута `name`, и поэтому просто отбрасывал то, что мы ввели.
...
а затем сохраним файл, откроем страницу в браузере, введем что-то и нажмем на кнопку "Создать", мы увидим, что URL страницы изменился - в него добавилось `?task=то, что вы ввели`. Как мы уже знаем, этот фрагмент URL называется query.
...
1. Вы помещаете в элемент `<form>` столько элементов `<input>`, сколько вам необходимо. У каждого тега `<input>` заполняете атрибут `name` (следите, чтобы он был уникален в пределах формы).
2. Вы указываете (при необходимости) атрибуты `action` и `method` тега `<form>`.
3. Когда вы сабмитите форму (подтверждаете отправку формы), браузер переходит по указанному вами в `action` URL (см. правила [выше](#как-браузер-понимает-куда-нужно-отправnять-данные)) и добавляет в него фрагмент query. То есть **по умолчанию данные формы передаются в URL**.
